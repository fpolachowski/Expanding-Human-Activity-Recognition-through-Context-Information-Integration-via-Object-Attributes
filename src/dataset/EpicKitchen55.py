...
class EpicKitchen55(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        temp = torch.tensor(self.vocab[record.activity], dtype=torch.long)
    ...
    def updateDF(self):
        ...
        self.df["label"] = self.df["activity"].apply(lambda x: self.activity_set.index(x.replace("-", " ")) if x.replace("-", " ") in self.activity_set else -1)
...
def loadDatasetDescription(annotation_file):
    with open(annotation_file) as f:
        ...
        df = df[["participant_id", "video_id", "start_frame", "stop_frame", "verb", "verb_class", "noun", "noun_class", "activity"]]
        return df
...
def prepare_data(batch_size=32, debug=False):
    """ Prepare data.
    """
    train_dataset = EpicKitchen55("train", mean=[0.3794, 0.3206, 0.2973], std=[0.2150, 0.2096, 0.2016])
    test_dataset = EpicKitchen55("test", mean=[0.3958, 0.3278, 0.3068], std=[0.2193, 0.2103, 0.2025], activity_set=train_dataset.activity_set, vocab_encoder=train_dataset.vocab_encoder, tokenized_vocab=train_dataset.tokenized_vocab)
